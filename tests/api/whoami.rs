...
async fn whoami_logged_in_test() -> Result<(), Error> {
    ...
    let whoami = logged_in_client.get("/api/v1/whoami".to_string(), Some(vec![("CloudFront-Viewer-Country-Name", "Iceland")])).await;
    ...
    assert_eq!(json["avatar_url"], "https://i1.sndcdn.com/avatars-000460644402-0iiiub-t500x500.jpg");
    assert_eq!(json["is_subscriber"], true);
    assert_eq!(json["subscription_type"], "mdn_plus_5m", "Subscription type wrong");
    Ok(())
}
...
async fn whoami_multiple_subscriptions_test() -> Result<(), Error> {
    ...
    Stubr::start_blocking_with(vec!["tests/stubs","tests/test_specific_stubs/whoami"], Config { port: Some(4321), latency: None, global_delay: None, verbose: Some(true) });
