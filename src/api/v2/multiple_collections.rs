use crate::api::common::{extract_user_id, get_document_metadata, Sorting};
...
pub async fn get_collections(
    ...
    let user: UserQuery = get_user(&mut conn_pool, extract_user_id(&user_id)?)?;
...
pub async fn create_multiple_collection(
    ...
    data.validate()?;
    let mut conn_pool = pool.get()?;
    let user = get_user(&mut conn_pool, extract_user_id(&user_id)?)?;
